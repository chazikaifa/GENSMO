...
$sql = 'SELECT SQL_CALC_FOUND_ROWS `order`.*,`number_exchange`.`product_number` FROM `order` LEFT JOIN `number_exchange` on `order`.`circuit_number`=`number_exchange`.`circuit_number` '. $condition .'ORDER BY `create_time` DESC';
if($limit != '' && $index != ''){
